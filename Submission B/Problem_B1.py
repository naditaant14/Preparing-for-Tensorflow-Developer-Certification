...
def solution_B1():
    ...
    model.fit(X, Y, epochs=5000, callbacks=[callback])  # Increase epochs to allow for more training iterations

    # Evaluate with additional data points
    X_test = np.array([-2.0, 10.0])
    print("Model predictions for [-2.0, 10.0]:", model.predict(X_test))
